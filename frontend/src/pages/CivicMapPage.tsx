...
// Static fallback ward data — always available immediately, no API wait needed
    ...
    const loadWards = async () => {
      try {
        ...
        if (!controller.signal.aborted) {
          setWards(res.wards || STATIC_WARDS);
        }
      } catch (err) {
        // Static wards already loaded — silent fallback
        console.log("Using static ward data (API unreachable)");
      }
    };
    loadWards();
    // Load policies for default selected ward
    handleSelectWard(STATIC_WARDS[0]);
